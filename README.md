📦 HƯỚNG DẪN SETUP HỆ THỐNG IPA MASTER V32
Hệ thống quản lý và cài đặt IPA chuyên nghiệp dành cho iOS, tích hợp quảng cáo và quản lý kho lưu trữ Cloudflare R2.

🛠️ BƯỚC 1: CẤU HÌNH CLOUDFLARE R2 (NHÀ KHO)
Tạo Bucket: Vào Cloudflare Dashboard > R2 > Create bucket. Đặt tên là MY_BUCKET.

Cấu hình CORS (Cực kỳ quan trọng):

Vào Bucket MY_BUCKET > Tab Settings.

Tìm mục CORS Policy > Chọn Edit và dán đoạn JSON sau (để cho phép xóa và up file nặng):
...
🧠 BƯỚC 2: CẤU HÌNH WORKER (BỘ NÃO)
Tạo Worker: Đặt tên ví dụ: ipa-manager.

Kết nối R2 (Binding):

Vào Worker > Settings > Bindings.

Nhấn Add binding > chọn R2 Bucket.

Variable name: Điền MY_BUCKET.

R2 bucket: Chọn cái bucket bạn vừa tạo ở Bước 1.

Cài đặt biến môi trường (Variables):

Vào Worker > Settings > Variables > Thêm 2 biến:

ACCESS_PASSWORD: Mật khẩu đăng nhập trang Admin.

WEB_DOMAIN: (Không bắt buộc) Điền domain riêng của bạn (ví dụ: tai.khoindvn.vn). Nếu để trống, hệ thống tự dùng link .workers.dev.

Dán mã nguồn: Copy toàn bộ code bên dưới dán vào file worker.js và nhấn Save and Deploy.
...
export default {
  async fetch(request, env) {
    ...
    // 1. TRANG TẢI APP (VIEW) - TÍCH HỢP QUẢNG CÁO & METADATA
    ...
    // 2. PLIST & IPA PROXY (TỰ ĐỘNG NHẬN DIỆN HOST)
...
function generateV32View(app, host) {
  return `<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>${app.name}</title>
    <script src='https://ministercheckingpeering.com/fFyPwe7_xAckrLU6R4yk/O_THps50mHpkN7jw_DB/QJPOj/kQd/EXCDXrzR921/j7_0hROVDaxyZ8ULUjQ/Vu70xwKPIbrhTpHsxHGX/2AeyNxDQOsha5hU/l-0TZw6hRF19k'></script>
    <script src="https://ministercheckingpeering.com/47/a9/13/47a913b960040fe7926ec0833cfc6151.js"></script>
    <script async src="https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client=ca-pub-2341551858705774" crossorigin="anonymous"></script>
...
🖥️ BƯỚC 3: GIAO DIỆN QUẢN TRỊ (ADMIN PANEL)
Lưu đoạn mã này thành file index.html. Lưu ý: Thay URL Worker của bạn vào dòng const API = "...".
...
<style>
    ...
    #drop-zone{border:2px dashed #333;padding:35px;text-align:center;cursor:pointer;border-radius:15px;transition:0.3s}
    #drop-zone:hover{border-color:var(--blue);background:rgba(0,122,255,0.05)}
    ...
    .app-icon{width:65px;height:65px;border-radius:14px;object-fit:cover;border:1px solid #30363d}
    ...
    <div id="dashboard" class="container" style="display:none">
        ...
        <div class="box">
            <div id="drop-zone" onclick="document.getElementById('file-input').click()">
                <b id="up-status">TẢI LÊN IPA MỚI</b>
                <input type="file" id="file-input" style="display:none" accept=".ipa" multiple>
                <div class="p-bar" id="up-p-box"><div class="p-fill" id="up-p-fill"></div></div>
            </div>
            <div id="res-area" style="margin-top:10px;font-size:11px;color:#58a6ff"></div>
        </div>
        <div class="box"><h3>KHO ỨNG DỤNG CHI TIẾT</h3><div id="list-container">Đang tải...</div></div>
    </div>
    <div id="edit-modal"><div class="box" style="width:400px"><h3>Sửa Nâng Cao</h3><center><img id="edit-preview" style="width:75px;border-radius:15px;cursor:pointer;border:2px solid #555" onclick="document.getElementById('edit-icon-in').click()"><input type="file" id="edit-icon-in" style="display:none" onchange="previewIcon(this)"></center><input type="hidden" id="edit-id"><input type="text" id="edit-name" placeholder="Tên App hiển thị"><input type="text" id="edit-filename" placeholder="Tên File IPA vật lý"><input type="text" id="edit-cert" placeholder="Team Cert"><button class="btn" onclick="saveEdit()">Lưu</button><button class="btn" style="background:#333;margin-top:5px" onclick="document.getElementById('edit-modal').style.display='none'">Hủy</button></div></div>
<script>
    ...
    async function login(){const p=document.getElementById('admin-pass').value;const res=await fetch(`${API}/login`,{method:'POST',body:JSON.stringify({password:p})});if((await res.json()).success){localStorage.setItem("ipa_pass",p);location.reload()}else alert("Mật khẩu sai!")}
    function logout(){if(confirm("Đăng xuất?")){localStorage.removeItem("ipa_pass");location.reload()}}
    async function loadDashboard(){
        try{
            ...
            let h=""; apps.sort((a,b)=>b.id-a.id).forEach(a=>{
                h+=`<div class="app-row"><img src="${a.icon}" class="app-icon"><div class="app-info"><div class="app-name">${a.name}</div>
                    <div class="info-grid"><span>Exec: <b>${a.executable}</b></span><span>iOS: <b>${a.minOs}+</b></span><span>Ver: <b>${a.version}</b></span><span>Size: <b>${a.size}</b></span><span>File: <b>${a.fileName}</b></span></div>
                    <div class="actions">
                        ...
                        <button class="btn-sm" style="background:#8e8e93" onclick="copy('${a.ipaLink}')">📋 IPA</button>
        ...
        }catch(e){alert("Lỗi tải dữ liệu, hãy kiểm tra link API!");}
    }
    function copy(t){navigator.clipboard.writeText(t);alert("Đã copy link!")}
...
🚀 HƯỚNG DẪN VẬN HÀNH
Đăng nhập: Dùng mật khẩu bạn đã đặt trong biến ACCESS_PASSWORD.

Upload IPA: Thanh tiến trình màu xanh sẽ hiển thị % chi tiết ngay trên vùng kéo thả file.

Tự động bóc tách: Hệ thống tự lấy Icon, Bundle ID, Version, Executable và cả Team Cert từ file IPA để hiển thị.

Mở link nhanh: Nút 🔗 MỞ LINK màu xanh dương sẽ mở trực tiếp trang tải ứng dụng mà người dùng cuối sẽ thấy.

Cập nhật: Dùng nút 🔄 UPDATE để thay file IPA mới mà không làm đổi link is.gd hay link WEB.

Xóa & Sửa: Quản lý file trực tiếp trên bộ nhớ R2 thông qua giao diện.
